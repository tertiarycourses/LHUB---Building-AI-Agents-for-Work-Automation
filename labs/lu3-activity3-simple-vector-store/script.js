/* =====================================================================
   ...
   ---------------------------------------------------------------------
   The chatbot calls your n8n RAG webhook. You set the URL in the "Lab
   configuration" panel on the page, so you never have to edit this file.

     Activity 3   ->  .../webhook/cookbake-course-enquiry       (Simple Vector Store)
     Activity 3b  ->  .../webhook/cookbake-course-enquiry-rag   (Pinecone)

   THE OFFLINE FALLBACK — read this before you trust an answer.
   If the webhook is missing or unreachable, the widget answers from a local
   keyword search over the course list further down this file. That lets you
   demo the page with no n8n at all, and it is also a trap: a completely broken
   RAG pipeline still produces confident, correct-looking answers.
   So every fallback answer is labelled, loudly, as NOT coming from your agent.
   Never judge your RAG by an answer carrying that label.
   ===================================================================== */

const CONFIG = {
  STORAGE_KEY: "cookbake.webhookUrl",
  DEFAULT_WEBHOOK_URL: "https://n8n.tertiarytraining.com/webhook/cookbake-course-enquiry",
...
const WebhookConfig = (() => {
  ...
  function render() {
    ...
    if (!url) {
      status.textContent =
        "No webhook URL set — the chatbot will answer from the offline demo data, not from your agent.";
      status.className = "config-status is-warn";
    } else if (url.includes("/webhook-test/")) {
      status.textContent =
        "Test URL detected. Arm “Listen for test event” in n8n before each message — it accepts one request only.";
      status.className = "config-status is-warn";
    } else if (url.includes("/webhook/")) {
      status.textContent = "Production URL detected. The workflow must be Active in n8n.";
      status.className = "config-status is-ok";
    } else {
      status.textContent = "This does not look like an n8n webhook URL.";
      status.className = "config-status is-warn";
    }
  }
...
const CookBakeChat = (() => {
  ...
  async function send(text) {
    ...
    let reply, offlineReason = null;

    if (!url) {
      offlineReason = "No webhook URL is set in the Lab configuration panel.";
      reply = localAnswer(text);
    } else {
      try {
        reply = await callWebhook(url, text);
      } catch (err) {
        console.warn("Webhook failed, using offline fallback:", err);
    ...
    // An unlabelled fallback answer is worse than no answer: it makes a dead
    // RAG pipeline look alive. Say so, every time, before the answer itself.
    if (offlineReason) {
      addMsg("bot",
        "⚠️ **Offline demo answer — this did NOT come from your n8n agent.**\n" +
        offlineReason + "\n\n" +
        "The reply below is a local keyword lookup. It proves nothing about your vector store.",
        "msg--offline");
    }
    addMsg("bot", reply);
  }
